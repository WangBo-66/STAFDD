...
def main(args):
    ...
    metrics_xlsx = os.path.join(experiment_dir, 'metrics.xlsx')  # 仅在 pandas 可用时写入
    # 如果没有 pandas，会用 csv 回退方案

    # 初始化 metrics 容器（将在每个 epoch 后追加）
    metrics_list = []
    ...
    # 如果没有 pandas，先把 CSV 写入头（回退方案）
    if not _HAS_PANDAS:
        with open(metrics_csv, 'w', newline='') as f:
            w = csv.writer(f)
            w.writerow(['epoch', 'train_loss', 'train_acc', 'val_loss', 'val_acc', 'epoch_time', 'lr'])
    ...
    for epoch in range(args.epochs):
        ...
        with open(error_csv, 'a', newline='') as f:
            ...
            for filename, track_id, pred, actual in train_errors:
                csv_writer.writerow([epoch + 1, 'train', filename, track_id, pred, actual])
        ...
        with open(error_csv, 'a', newline='') as f:
            ...
            for filename, track_id, pred, actual in val_errors:
                csv_writer.writerow([epoch + 1, 'val', filename, track_id, pred, actual])
        ...
        metrics_entry = {
            ...
            'epoch_time': float(epoch_duration),
            'lr': float(current_lr)
        }
        metrics_list.append(metrics_entry)

        # 尝试用 pandas 写 CSV + XLSX（如果可用），否则回退到 csv 模块追加
        if _HAS_PANDAS:
            try:
                ...
                # 尝试写 xlsx（需要 openpyxl 或类似引擎）
                try:
                    df.to_excel(metrics_xlsx, index=False)
                except Exception as e:
                    # 如果写 xlsx 失败（缺少 openpyxl 等），只写 csv
                    print(f"写入 xlsx 失败: {e}，已保存 CSV。")
            except Exception as e:
                print(f"保存 metrics 到 pandas 出错：{e}. 使用回退 CSV 写法。")
                # 回退：用 csv 模块覆盖写一次
                with open(metrics_csv, 'w', newline='') as f:
                    w = csv.writer(f)
                    w.writerow(['epoch', 'train_loss', 'train_acc', 'val_loss', 'val_acc', 'epoch_time', 'lr'])
                    for m in metrics_list:
                        w.writerow([m['epoch'], m['train_loss'], m['train_acc'], m['val_loss'], m['val_acc'], m['epoch_time'], m['lr']])
        else:
            # 回退方案：直接追加到 CSV（效率足够）
            with open(metrics_csv, 'a', newline='') as f:
                w = csv.writer(f)
                w.writerow([metrics_entry['epoch'],
                            metrics_entry['train_loss'],
                            metrics_entry['train_acc'],
                            metrics_entry['val_loss'],
                            metrics_entry['val_acc'],
                            metrics_entry['epoch_time'],
                            metrics_entry['lr']])
        # --------- end 保存逻辑 ---------
...
def train(data_loader, model, criterion, optimizer, device, dataset, epoch):
    ...
    training_acc = 100.0 * num_correct / num_total if num_total > 0 else 0.0
...
def evaluate(data_loader, model, criterion, device, dataset, epoch):
    ...
    val_acc = 100.0 * num_correct / num_total if num_total > 0 else 0.0
...
def get_lr(optimizer):
    """Return current learning rate (first param group)."""
    try:
